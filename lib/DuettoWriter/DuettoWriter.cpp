...
void DuettoWriter::compileAllocation(const Value* callV, const Value* size, const Value* numElements)
{
	...
	for(;it!=itE && castedType==NULL;++it)
	{
		...
		else if(const IntrinsicInst* ci = dyn_cast<IntrinsicInst>(*it))
		{
			//Support duetto.cast.user
			if(ci->getIntrinsicID() == Intrinsic::duetto_cast_user)
				castedType = it->getType();
...
bool DuettoWriter::canBeCalledIndirectly(const Function * f) const 
{
	...
	{
		bool ans = false;
		for (Function::const_use_iterator it = f->use_begin(); it != f->use_end() && !ans; ++it)
		{
			if (const CallInst * ci = dyn_cast<const CallInst>(*it))
				if (ci->getCalledFunction())
					continue;
			if (const InvokeInst * ci = dyn_cast<const InvokeInst>(*it))
				if (ci->getCalledFunction())
					continue;
			ans = false;
		}
...
bool DuettoWriter::isInlineable(const Instruction& I) const
{
	...
	else if(I.getOpcode()==Instruction::BitCast)
	{
		...
		for(;it!=itE;++it)
		{
			if(!LoadInst::classof(*it) && !StoreInst::classof(*it))
				return false;
		}
		return true;
	}
