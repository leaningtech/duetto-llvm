...
DuettoWriter::POINTER_KIND DuettoWriter::dfsPointerKind(const Value* v, std::map<const Value*, POINTER_KIND>& visitedPhisOrArguments) const
{
	...
	if ( const Argument * arg = dyn_cast<const Argument>(v) )
	{
		...
		for( Function::const_use_iterator iter = F->use_begin(); iter != F->use_end() && current->second != REGULAR; ++iter)
		{
			...
			if (const CallInst * call = dyn_cast<const CallInst>(*iter) )
			{
			...
			else if (const InvokeInst * call = dyn_cast<const InvokeInst>(*iter) )
			{
				if (call->getCalledFunction() == F)
				{
					assert (F->getArgumentList().size() == call->getNumArgOperands() || (F->dump(),false) );
...
uint32_t DuettoWriter::getPointerUsageFlags(const llvm::Value * v) const
{
	...
	if (iter == pointerUsageMap.end())
	{
		...
		for (Value::const_use_iterator it = v->use_begin(); it != v->use_end(); ++it)
		{
			...
			if (const StoreInst * I = dyn_cast<const StoreInst>(*it) )
			{
			...
			{
#ifdef DUETTO_DEBUG_POINTERS
				print_debug_pointer_uknown(v,*it,"getPointerUsageFlags");
#endif //DUETTO_DEBUG_POINTERS
				ans |= POINTER_UNKNOWN;
			}
		}
...
uint32_t DuettoWriter::dfsPointerUsageFlagsComplete(const Value * v, std::set<const Value *> & openset) const
{
	...
	for (Value::const_use_iterator it = v->use_begin(); it != v->use_end(); ++it)
	{
		...
		else if (const CallInst * I = dyn_cast<const CallInst>(*it))
		{
			// Indirect calls require a finer analysis
			f |= POINTER_UNKNOWN;
		}
		else if (isa<const InvokeInst>(*it))
		{
			//TODO same thing as CallInst.
			f |= POINTER_UNKNOWN;
		}
		else if (isa<const ReturnInst>(*it))
		{
