...
void CheerpWriter::compileMemFunc(const Value* dest, const Value* src, const Value* size, COPY_DIRECTION copyDirection)
{
                                        ...
                                        const Value* offset)
{
	...
	case Type::StructTyID:
	{
		...
		for(StructType::element_iterator E = st->element_begin(),
		                EE = st->element_end();
		                E != EE; ++E)
		{
...
void CheerpWriter::compileDowncast(ImmutableCallSite callV)
{
	...
	assert(isa<Constant>(callV.getArgument(1)));
